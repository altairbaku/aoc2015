        ...
        for i in range(len(cur_molecule) - len_find + 1):
            if cur_molecule[i:i+len_find] == find:
                for replace in replace_dict[find]:
                    ...
                    cost = molecule_len - hamming_distance(medicine_molecule[:len(new_molecule)],new_molecule)
                    if new_molecule not in distinct_molecules and len(new_molecule) <= molecule_len and cost <= cur_molecule_details[0]:
